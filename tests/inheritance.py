    ...
    class header(Band):
        elements = [
            Label(top=0 * mm, left=20 * mm, height=10 * mm, width=145 * mm, 
                border=(None, None, QtGui.QPen(QtGui.QBrush("black"), 3), None),
...
class InheritedReport(BaseReport):
    ...
    detail = DetailBand(
        height = 5 * mm,
        elements = [
            Function(top=0 * mm, left=0 * mm, height=5 * mm, width=30 * mm, func=lambda x: x[0]),
            Function(top=0 * mm, left=100 * mm, height=5 * mm, width=30 * mm, func=lambda x: x[1])
            ])
